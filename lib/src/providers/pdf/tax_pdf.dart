...
class CreateTaxPdf {
  ...
  UserModel user;
  CreateTaxPdf({this.invoice, this.user});
        ...
        build: (context) => [
          pw.Directionality(
            ...
            child: pw.Column(children: [
              ...
              //   child: pw.Center(child: pw.Text(invoice.btb_title.ar)),
              //   // child: pw.Center(child: pw.Text("فاتورة ضريبية")),
    ...
    return pw.Column(
        children:[
            ...
            children: [
              ...
              pw.Expanded(
                child: pw.Column(
                  children: [
                    _infoChild(
                        label: "رقم الفاتورة", info: invoice.invoiceNo.toString()),
            ...
            children: [
              pw.Expanded(
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    _infoChild(label: "السجل التجاري", info: "${invoice.customer.segal}"),
                  ],
                ),
              )
            ],
          ...
          pw.Container(
            width: double.infinity,
            padding: pw.EdgeInsets.all(5),
            child: pw.Center(child: pw.Text("العنوان الوطني للمؤسسة")),
            decoration: pw.BoxDecoration(
            ...
            children: [
              pw.Expanded(
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    _infoChild(label: "المدينة", info: "${user.company.companycity}"),
                    _infoChild(label: "الرمز البريدي", info: "${user.company.companypostcode}"),
                    _infoChild(label: "الشارع", info: "${user.company.companystreet}"),
                  ],
                ),
              ),
              pw.Expanded(
                child: pw.Column(
                  children: [
                    _infoChild(label: "الدولة", info: "${user.company.companycountry}"),
                    _infoChild(label: "الحي", info: "${user.company.companystate}"),
                    _infoChild(label: "رقم المبنى", info: "${user.company.companybuilding}"),
                  ],
                ),
              )
            ],
          ...
          pw.Container(
            width: double.infinity,
            padding: pw.EdgeInsets.all(5),
            child: pw.Center(child: pw.Text("العنوان الوطني للعميل")),
            decoration: pw.BoxDecoration(
              border: pw.Border.all(width: 1),
            ),
          ),
            ...
            children: [
              pw.Expanded(
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    _infoChild(label: "المدينة", info: "${invoice.customer.city}"),
                    _infoChild(label: "الرمز البريدي", info: "${invoice.customer.postcode}"),
                    _infoChild(label: "الشارع", info: "${invoice.customer.street}"),
                  ],
                ),
              ),
              pw.Expanded(
                child: pw.Column(
                  children: [
                    _infoChild(label: "الدولة", info: "${invoice.customer.country}"),
                    _infoChild(label: "الحي", info: "${invoice.customer.state}"),
                    _infoChild(label: "رقم المبنى", info: "${invoice.customer.building}"),
                    _infoChild(label: "الجوال", info: "${invoice.customer.mobile}"),
                  ],
                ),
              )
            ],
  ...
  pw.Widget _amountInfoAmount(var amount, {bool isString = false}) {
    return pw.Container(
      width: 70,
      decoration: pw.BoxDecoration(border: pw.Border.all(width: .5)),
